...
class ImageWebpConverter
{
    ...
    public function convert(string $sourcePath, int $maxBytes = self::MAX_BYTES): string
    {
        if (! class_exists(Imagick::class) || Imagick::queryFormats('WEBP') === []) {
            throw new RuntimeException('ImageMagick WebP support is not available.');
        }
        ...
        try {
            ...
            while (true) {
                foreach (self::QUALITIES as $quality) {
                    ...
                    if (strlen($webp) <= min(self::MAX_BYTES, $maxBytes)) {
        ...
        throw new RuntimeException('The image could not be compressed below 1.8 MiB.');
    }
